...

{
  ...
  m_heta = tfserv->make<TH1F>("eta","Track pseudorapidity",120,-3,3);
  ...
  m_chisq = tfserv->make<TH1F>("chisq","Track Chi2",50,0.,200.);
  m_chisq->GetXaxis()->SetTitle("chi2");  m_chisq->GetYaxis()->SetTitle("Tracks");
  m_chisqnorm = tfserv->make<TH1F>("chisqnorm","Track normalized Chi2",50,0.,50.);
  m_chisqnorm->GetXaxis()->SetTitle("normalized chi2");  m_chisqnorm->GetYaxis()->SetTitle("Tracks");
  m_ndof = tfserv->make<TH1F>("ndof","Track ndof",50,0.,200.);
  m_ndof->GetXaxis()->SetTitle("ndof");  m_ndof->GetYaxis()->SetTitle("Tracks");

  m_hptphieta = tfserv->make<TProfile2D>("ptphivseta","Average pt vs #phi vs #eta",40,-3.,3.,40,-M_PI,M_PI);
  ...
  m_hnhitphieta = tfserv->make<TProfile2D>("nhitphivseta","N valid hits vs #phi vs #eta",40,-3.,3.,40,-M_PI,M_PI);
  m_hnhitphieta->GetXaxis()->SetTitle("#eta");   m_hnhitphieta->GetYaxis()->SetTitle("#phi"); 
  m_hnlayerphieta = tfserv->make<TProfile2D>("nlayerphivseta","N layers vs #phi vs #eta",40,-3.,3.,40,-M_PI,M_PI);
  ...
  m_hphieta = tfserv->make<TH2D>("phivseta","#phi vs #eta",40,-3.,3.,40,-M_PI,M_PI);
  m_hphieta->GetXaxis()->SetTitle("#eta");   m_hphieta->GetYaxis()->SetTitle("#phi"); 

  if(m_runHisto) {
    m_ntrkvsorbrun = m_rhm.makeTProfile("ntrkvsorbrun","Number of tracks vs orbit number",1800,0.5,1800.*11223+0.5);
  }
   ...
   for(reco::TrackCollection::const_iterator it = tracks->begin();it!=tracks->end();it++) {
     ...
     if(m_dump) {
       edm::LogVerbatim("TrackDump") << it->quality(quality) << " "
				     ...
				     << it->hitPattern().trackerLayersWithoutMeasurement() << " ";

     }

     m_hnlosthits->Fill(it->hitPattern().numberOfLostTrackerHits());
     ...
     m_hnlostlayers->Fill(it->hitPattern().trackerLayersWithoutMeasurement());
