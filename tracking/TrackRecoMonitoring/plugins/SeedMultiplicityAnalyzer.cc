...
// $Id: SeedMultiplicityAnalyzer.cc,v 1.7 2011/09/27 08:49:03 venturia Exp $
//
//
...
void
SeedMultiplicityAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
    ...
    */    
	...
	for(TrajectorySeed::const_iterator hit = rechits.first; hit!= rechits.second; ++hit) {
	  ...
	  if(sphit) {
	    ...
	    TrajectoryStateOnSurface tsos = tsTransform.transientState( seed->startingState(), ttrhit->surface(), theMF.product());	    
